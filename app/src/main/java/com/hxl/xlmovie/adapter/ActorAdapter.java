...
public class ActorAdapter extends RecyclerView.Adapter<ActorAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        if (position == 0) {
            holder.ivActor.setImageURI(mDirectors.get(position).getAvatars().getMedium());
            holder.tvName.setText("导演");
            holder.tvActor.setText(mDirectors.get(position).getName());
        } else {
            holder.ivActor.setImageURI(mCasts.get(position - 1).getAvatars().getMedium());
            holder.tvName.setText(mCasts.get(position - 1).getName());
